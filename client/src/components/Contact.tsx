...
export default function Contact() {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      // Map the subject value to a human‑friendly label for the WhatsApp message
      ...
      // Compose the message according to the requested format.  Blank
      // lines separate the sections to improve readability in WhatsApp.
      const messageText = `${subjectLabel}\n\n${data.message}\n\n${data.name}\n${data.email}\n${data.taxId}`;
      // Target phone number for WhatsApp (Magnum Torque contact).  Use
      // international format without +.  If this number ever changes
      // consider extracting it to a configuration file or environment.
      const whatsappNumber = "554135036828";
      const encoded = encodeURIComponent(messageText);
      const whatsappUrl = `https://wa.me/${whatsappNumber}?text=${encoded}`;
      // Open WhatsApp in a new tab/window.  On mobile devices this
      // triggers the WhatsApp app if installed.
  ...
  return (
    <section id="contato" className="relative pt-8 pb-12 sm:pt-12 sm:pb-16 md:pt-16 md:pb-20 lg:pt-20 lg:pb-24 xl:pt-24 xl:pb-28 bg-gradient-to-br from-gray-50 via-white to-slate-50 overflow-hidden">
      ...
      <div className="absolute top-1/4 left-1/4 w-32 h-32 opacity-3">
        <div className="w-full h-full border border-primary/20 rotate-12 rounded-2xl"></div>
      </div>
      ...
      <div className="absolute inset-0 opacity-4">
        <div className="absolute top-20 left-10 sm:top-32 sm:left-40 w-2 h-2 bg-primary rounded-full animate-pulse"></div>
        <div className="absolute top-32 right-10 sm:top-48 sm:right-32 w-1.5 h-1.5 bg-gray-500 rounded-full"></div>
        <div className="absolute bottom-10 left-10 sm:bottom-20 sm:left-20 w-2 h-2 bg-primary rounded-full animate-pulse" style={{ animationDelay: '1s' }}></div>
        <div className="absolute bottom-20 right-10 sm:bottom-32 sm:right-40 w-1 h-1 bg-gray-500 rounded-full"></div>
      </div>
                ...
                <div className="relative z-10">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="group flex items-start p-4 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 hover:bg-white/10 transition-all duration-300">
                      ...
                      <div>
                        <h4 className="font-semibold mb-1 font-montserrat text-white">Endereço e Horário</h4>
